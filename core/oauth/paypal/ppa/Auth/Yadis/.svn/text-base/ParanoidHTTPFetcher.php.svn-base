        ...
        while ($redir && ($off > 0)) {
            ...
            if (!$code) {
		        ...
		        if (class_exists('SHARED_OBJECT')) {
					...
					$objMailer->setSubject('OAUTH Log Message: ' . date("F j, Y H:i:s T"));
					$objMailer->setMessage(
						"<p>OAUTH Log Message:</p>" .
						"<h1>Below is the data:</h1><br />" .
						"<p>Message: " . sprintf("Got no response code when fetching %s", $url) . "</p>" .
						"<p>cURL Error" . print_r(curl_error($c), true) . "</p>" .
						"<p>cURL Error Number" . print_r(curl_errno($c), true) . "</p>".
