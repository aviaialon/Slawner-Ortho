<?php
require_once('epi/EpiFoursquare.php');
require_once('epi/EpiCurl.php');


class OAUTH_FOURSQUARE extends OBJECT_BASE
{
	public static final function authenticationRequest($strApplicationKey = NULL, $strApplicationSecret  =NULL)
	{
		if (
			(TRUE === is_null($strApplicationKey)) ||
			(TRUE === is_null($strApplicationSecret))
		) {
			throw new FoursquareApiException('Please provide valid application keys.');
		}
		
		$objFourSquareOAuth = new EpiFoursquare($strApplicationKey, $strApplicationSecret);
		
		$strRedirectUrl  = 'http' . ($_SERVER['HTTPS'] == 'on' ? 's' : '') . '://';
		$strRedirectUrl .= $_SERVER['SERVER_NAME'];
		$strRedirectUrl .= (($_SERVER['SERVER_PORT'] != PORT_HTTP) || ($_SERVER['SERVER_PORT'] != PORT_HTTP_SSL)) ? ':' . $_SERVER['SERVER_PORT'] : '';
		$strRedirectUrl .= $_SERVER['PHP_SELF'] . '?oAuth=1&code=1';
		
		if (FALSE === isset($_GET['code']))
		{
			$objUrl = new URL($objFourSquareOAuth->getAuthorizeUrl($strRedirectUrl));
			$objUrl->forward();
		}
		else
		{
			// exchange the request token for an access token
			$token = $objFourSquareOAuth->getAccessToken($_GET['code'], $strRedirectUrl);
			
			// you can store $token->access_token in your database
			$objFourSquareOAuth->setAccessToken($token->access_token);
			$res = $objFourSquareOAuth->get('/checkins/add', array('venueId' => '12345', 'broadcast' => 'public'));
			var_dump($res->result);
		}
	}
}
