<?php
// https://github.com/jmathai/foursquare-async/wiki
class EpiCurlFourSquare
{
  const timeout = 3;
  static $inst = null;
  static $singleton = 0;
  ...
  function __construct()
  {
    if(self::$singleton == 0)
    {
      throw new Exception('This class cannot be instantiated by the new keyword.  You must instantiate it using: $obj = EpiCurlFourSquare::getInstance();');
    }
  ...
  public function addCurl($ch)
  {
    ...
    if($code === CURLM_OK || $code === CURLM_CALL_MULTI_PERFORM)
    {
      ...
      return new EpiCurlManagerFourSquare($key);
    }
    else
    {
      return $code;
  ...
  static function getInstance()
  {
    ...
    {
      self::$singleton = 1;
      self::$inst = new EpiCurlFourSquare();
    }
...
class EpiCurlManagerFourSquare
{
  private $key;
  private $epiCurl;
  ...
  {
    $this->key = $key;
    $this->epiCurl = EpiCurlFourSquare::getInstance();
  }
