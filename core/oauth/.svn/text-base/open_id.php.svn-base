...
/**
	...
	try {
		...
		$openid = new LightOpenID('http://legacy.com/');
		if(!$openid->mode) {
			if(isset($_GET['login'])) {
	...
	<?php
			...
			new dump($openid->validate());
			new dump($openid->getAttributes());
			new dump($openid);
...
class LightOpenID
{
    ...
    protected function request_curl($url, $method='GET', $params=array())
    {
        ...
        if($method == 'HEAD') {
			...
			# Debug here...
			# print($effective_url . '<br />' . $url); die;
...
class OPEN_ID extends LightOpenID {
	const OPEN_ID_AUTHENTICATION_MODE_CANCEL = 'cancel';
	...
	public function setIdentity($strIdentity = NULL) {
		$this->identity = $strIdentity;
	}
	
	public function setRequired($arrRequired = array()) {
		$this->required = (array) $arrRequired;
	}
