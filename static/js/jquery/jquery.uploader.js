...
qq.extend(qq.UploadHandlerForm.prototype, {
    ...
    _upload: function(id){
        ...
        if (!input){
            throw new Error(SLAWNER.APPLICATION.SLAWNER_UPLOADER.translate(
				'file with passed id was not added, or already uploaded or cancelled', 
				'fichier avec l\'id passé n\'a pas été ajouté ou déjà téléchargé ou annulé'
			));
        }
...
qq.FineUploaderBasic = function(o){
    ...
    this._options = {
        ...
        messages: {
			typeError: SLAWNER.APPLICATION.SLAWNER_UPLOADER.translate(
				'{file} has an invalid extension. Valid extension(s) are: {extensions}.', 
				'{file} possède une extension invalide. Extension valide (s) sont: {extensions}'
			),
            sizeError: SLAWNER.APPLICATION.SLAWNER_UPLOADER.translate(
				"{file} is too large, maximum file size is {sizeLimit}.", 
				"{file} est trop grand, la taille maximale des fichiers est {sizeLimit}."
			),
            minSizeError: SLAWNER.APPLICATION.SLAWNER_UPLOADER.translate(
				"{file} is too small, minimum file size is {minSizeLimit}.", 
				"{file} est trop petite, la taille de fichier minimale est {minSizeLimit}."
			),
            emptyError: SLAWNER.APPLICATION.SLAWNER_UPLOADER.translate(
				"{file} is empty, please select files again without it.", 
				"{file} est vide, veuillez sélectionnez les fichiers à nouveau sans ce ficher."
			),
            noFilesError: SLAWNER.APPLICATION.SLAWNER_UPLOADER.translate(
				"No files to upload.", 
				"Aucun de fichiers à telecharger."
			),
            onLeave: SLAWNER.APPLICATION.SLAWNER_UPLOADER.translate(
				"The files are being uploaded, if you leave now the upload will be cancelled.", 
				"Les fichiers sont téléchargés, si vous quittez maintenant le téléchargement sera annulé."
			)
        },
        retry: {
            enableAuto: false,
...
qq.FineUploader = function(o){
    ...
    qq.extend(this._options, {
        ...
        text: {
            uploadButton: SLAWNER.APPLICATION.SLAWNER_UPLOADER.translate(
				'Upload a file', 
				'téléchargez un fichier'
			),
            cancelButton: SLAWNER.APPLICATION.SLAWNER_UPLOADER.translate(
				'Cancel', 
				'Annuler'
			),
            retryButton: SLAWNER.APPLICATION.SLAWNER_UPLOADER.translate(
				'Retry', 
				'Essayer a nouveau'
			),
            failUpload: SLAWNER.APPLICATION.SLAWNER_UPLOADER.translate(
				'Upload failed', 
				'échec du téléchargement'
			),
            dragZone: SLAWNER.APPLICATION.SLAWNER_UPLOADER.translate(
				'Drop files here to upload', 
				'Déposer des fichiers ici pour télécharger'
			),
            dropProcessing: SLAWNER.APPLICATION.SLAWNER_UPLOADER.translate(
				'Processing dropped files...', 
				'Traitement des fichiers déposés ...'
			),
            formatProgress: SLAWNER.APPLICATION.SLAWNER_UPLOADER.translate(
				"{percent}% of {total_size}", 
				"{percent}% de {total_size}"
			),
            waitingForResponse: SLAWNER.APPLICATION.SLAWNER_UPLOADER.translate(
				"Processing...", 
				"Traitement..."
			)
        },
        ...
        classes: {
            ...
            successIcon: null,
            failIcon: '/static/images/list-cross.png',
...
qq.extend(qq.FineUploader.prototype, {
    ...
    _setupDragAndDrop: function() {
		...
		try {
			this._options.callbacks.onReady();
		} catch (e) {}
    ...
    _onComplete: function(id, fileName, result, xhr){
        ...
        if (result.success){
            ...
            if (this._classes.failIcon) {
				...
				$(this._find(item, 'finished')).find('img.jq-slawner-error-icon').remove();
				img = $('<img/>').attr({src: this._classes.failIcon}).addClass('jq-slawner-error-icon');
				img.prependTo($(this._find(item, 'finished')));
            }
...
SLAWNER.APPLICATION.SLAWNER_UPLOADER = 
{
	lang: LANG || 'en',
	fileCount : 0,
	addedFiles : 0,
	fileLimit : 1,
	initialise: function(configObject)	 
	{
		if (typeof configObject.fileLimit !== "undefined")
		{
			SLAWNER.APPLICATION.SLAWNER_UPLOADER.fileLimit = Number(configObject.fileLimit);	
			delete configObject.fileLimit;
		}
		
		return (new qq.FineUploader(configObject));	
	},
	
	translate: function(strEngVal, strFrVal)
	{
		var strRetData 	= strEngVal;
		var strlang 	= ((SLAWNER.APPLICATION.SLAWNER_UPLOADER.lang) || ('en'));
