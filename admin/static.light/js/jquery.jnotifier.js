...
(function ($) {
    $.jnotify = function (title, text, pictureSrc, options) {
		...
		if ((Number($('div.message-wrap', stackContainer).length) + 1) > Number(options.maxNotices)) {
			$('div.message-wrap', stackContainer).slice(0, ($('div.message-wrap', stackContainer).length + 1) - Number(options.maxNotices)).effect('drop', {
				direction: 'right'																																	   
			}, 500, function() {
				$(this).remove();	
			});							
			/*
			$('div.message-wrap', stackContainer).slice(0, ($('div.message-wrap', stackContainer).length + 1) - Number(options.maxNotices)).fadeOut('slow', function() {
				$(this).remove();																	 
			});
			*/
		} 
		
		messageWrap.appendTo(stackContainer).delay(10).fadeIn('slow');
		//messageWrap.appendTo(stackContainer).effect('bounce', {}, 1, function() {});
		
		
		/*
		messageWrap.appendTo(stackContainer).fadeTo('slow', 0.75).hover(function() {
		...
		*/
        ...
        if (options.lifeTime > 0) {
            setTimeout(function () {
				/*
				$(messageWrap).effect('pulsate', {times: 2}, 300, function() {
        ...
        if (options.customClass != '') {
            messageWrap.addClass(options.customClass)
        }
