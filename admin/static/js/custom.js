$(function() {
	...
	$('#fileManager').elfinder({
			url : 'php/connector.php?authToken=' + _AUTH_TOKEN,
			toolbar : [
				['back', 'reload'],
				['select', 'open'],
	...
	$('.acc .head:eq(0)').show().css({color:"#abb3ba"});
	
	$(".acc .head").click(function() {	
		$(this).css({color:"#abb3ba"}).next("div.menu_body").slideToggle(300).siblings("div.menu_body").slideUp("slow");
		$(this).siblings().css({color:"#d5d5d5"});
	});
	...
	$.fn.simpleTabs = function(){ 
		...
		if ($(this).find("ul.tabs li.activeTab").length) 
		{
			$(this).find("ul.tabs li.activeTab").show();  //Show the current active tab
			$(this).find("div#" + $(this).find("ul.tabs li.activeTab").attr('rel')).css({'display': 'block'}); //Show the active contest
		}
		else	
		{
			$(this).find("ul.tabs li:first").addClass("activeTab").show(); // otherwise Activate first tab
...
$(function () {
	...
	catch (e)
	{
		if (typeof console !== "undefined")
				console.log(' -- Chart Error: ' + e.toString());	
	}
});
	...
	$(function () {
		...
		{
			if (typeof console !== "undefined")
				console.log(' -- Chart Error 2: ' + e.toString());	
		}
	});
	...
	$(function () {
		...
		{
			if (typeof console !== "undefined")
				console.log(' -- Chart Error 3: ' + e.toString());	
		}
	});
	...
	$(function () {
		...
		{
			if (typeof console !== "undefined")
				console.log(' -- Chart Error 4: ' + e.toString());	
		}
	});
...
$(document).ready(function() {
	...
	gbox = {
		...
		_show: function(options) {
			...
			// set the drag handle here
			$( "#gbox" ).draggable({ 
				"handle": "#gbox #gbox_content div.head",
				"cursor": "crosshair",
				"opacity": 0.35
			});
		},
	...
	$('div#gbox_content a[rel=cancel], div#gbox_content input[rel=cancel]').live('click', function(event) {
		event.preventDefault();
		gbox.hide();
	});
});

/* This methods prevent leaving a page if changes has occured */
var CHANGE_PROMPT = {
	// This variable holds the boolean if the user can leave the current page
	_blnCanLeave : true,
	
	// This variable holds the prompt text
	_strPromptText : 'You Have Unsaved Changes, Are You Sure You Want To Leave This Page?',
	
	// This variable just holds a default copy of the change text
	_strDefaultPromtText : 'You Have Unsaved Changes, Are You Sure You Want To Leave This Page?',
	
	// This variable holds a function to execute when prompted
	_fnOnBeforePrompt : null,
	
	// This variable sets a change flag
	addChangeFlag : function (strText, fn) {
		
		CHANGE_PROMPT._blnCanLeave = false;
		
		if (typeof strText !== "undefined")	 {
			CHANGE_PROMPT._strPromptText = 	strText;
		}
		
		if (typeof fn !== "undefined")	 {
			CHANGE_PROMPT._fnOnBeforePrompt = 	fn;
		}
	},
	
	// This method removes the change flag
	removeFlag : function() {
		CHANGE_PROMPT._blnCanLeave = true;
		CHANGE_PROMPT._strPromptText = CHANGE_PROMPT._strDefaultPromtText;
	},
	
	_hidePageLoader : function() {
		$('#page').show();
		$('#pageLoader').hide();	
	},
	
	// This method is fired when the unload request has executed
	_unloadRequest : function(event) {
		
		 event = event || window.event;  
		 
		if (false === CHANGE_PROMPT._blnCanLeave) 
		{
			// OnBeforePrompt Function Execution
			if (typeof CHANGE_PROMPT._fnOnBeforePrompt !== "undefined")	 {
				CHANGE_PROMPT._fnOnBeforePrompt(event);
			}
			
			// For IE and Firefox
			if (event)
			{
				event.returnValue = jConfirm(CHANGE_PROMPT._strPromptText, 'Confirmation Dialog', function(blnResults) {
					if (Boolean(blnResults)) {
						CHANGE_PROMPT._hidePageLoader();
					}
					return (Boolean(blnResults));
				}) || false;
			}
			// For Safari
			//return "You have unsaved changes.";
			
			return jConfirm(CHANGE_PROMPT._strPromptText, 'Confirmation Dialog', function(blnResults) {
				if (Boolean(blnResults)) {
					CHANGE_PROMPT._hidePageLoader();
				}
				return (Boolean(blnResults));
...
// Bind the before unload event
$(window).bind('beforeunload', function(event) {
	return CHANGE_PROMPT._unloadRequest(event);
}); 

/* same as site_url() in php
------------------------------------------------------------------------- */
function site_url(__url__) {
	/*
	var strQueryString = '';
	var siteUrl 	 = url.replace(_BASE_URL, ""); 		// We replace the base site URL because we'll add it later on
	var arrUrlParts	 = siteUrl.split('?'); 				// We also remove the URL parameters because we'll add them again after
	var finalSiteUrl = siteUrl[0];						// Set the URL to the first part
	var arrUrlParams = (arrUrlParts.length > 1 ? arrUrlParts.split('&') : new Array()); // Split the array parts
	
	for(i=0; i <= arrUrlParams.length; i++)
	{
		if (typeof arrUrlParams[i] !== "undefined")
		{
			arrUrlQueryPart = arrUrlParams[i].split('=');
			if (arrUrlQueryPart.length == 2 && arrUrlQueryPart[0].toLowerCase() !== 'authtoken')
			{
				finalSiteUrl += ((url.indexOf("?") != -1) ? '&' : '?') + arrUrlQueryPart[0] + '=' + arrUrlQueryPart[1];
			}
		}
	}
	// Add the auth token
	finalSiteUrl += ((finalSiteUrl.indexOf("?") != -1) ? '&' : '?') + 'authToken=' + _AUTH_TOKEN;
	
	alert(finalSiteUrl); return false;
	*/
...
$(window).ready(function() {
	...
	$("ul#menu li a:not(.active)").unbind('click');
	$("ul#menu li a:not(.active)").live('click', function(event) {
		$('#page').hide();
		$('#pageLoader').show();
	});
	
	$("ul.sub").css({'display': 'none'});
	$("ul#active_sub_menu").css({'display': 'block'});
});